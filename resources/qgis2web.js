...
var map = new ol.Map({
    ...
    layers: layersList,
    view: new ol.View({
         maxZoom: 17, minZoom: 8.5
    })
});

//initial view - epsg:3857 coordinates if not "Match project CRS"
map.getView().setCenter([930000, 5900000]);
map.getView().setZoom(8.8);
...
function createPopupField(currentFeature, currentFeatureKeys, layer) {
    var title = currentFeature.get('Spalte1') || 'No title';
    var url = currentFeature.get('URL') || '#';

    return `
        <div class="popup-subtitle">${title}</div>
        <div class="popup-link"><a href="${url}" target="_blank" rel="noopener">Website</a></div>
    `;
}
            ...
            if (typeof clusteredFeatures !== "undefined") {
                if (doPopup) {
                    for(var n=0; n<clusteredFeatures.length; n++) {
                        ...
                        popupText += `<div class="popup-title">${layer.get('popuplayertitle')}</div>`;
...
function onSingleClickFeatures(evt) {
    ...
    map.forEachFeatureAtPixel(pixel, function (feature, layer) {
        ...
        // Make sure the layer is interactive (or allow all if undefined)
